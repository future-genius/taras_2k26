 ...
 */
export const onEmailJobCreated = functions.firestore.onDocumentCreated('email_jobs/{jobId}', async (event) => {
 ...
 */
export const onCertificateJobCreated = functions.firestore.onDocumentCreated('certificate_jobs/{jobId}', async (event) => {
    const snap = event.data;
    if (!snap)
        return;
    const data = snap.data();
