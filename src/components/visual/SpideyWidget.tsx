...
export const SpideyWidget: React.FC = () => {
  ...
  // Trigger Spidey Web Shot & Play Spectacular Spider-Man Theme Song
  const handleSpideyClick = () => {
    setIsWebShooting(true);

    if (!audioRef.current) {
      audioRef.current = new Audio('/audio/spectacular-spiderman.mp3');
  ...
  return (
    <div className="fixed bottom-20 left-4 lg:bottom-6 lg:left-6 z-40 flex flex-col items-center pointer-events-auto">
      ...
      <motion.div
        animate={{
          rotate: isPlaying ? [-12, 12, -12] : [-7, 7, -7],
          y: isPlaying ? [0, 8, 0] : [0, 6, 0],
        }}
        transition={{
      ...
      >
        ...
        <div className="absolute -inset-4 rounded-full pointer-events-none flex items-center justify-center">
          <motion.div
            animate={{
              scale: isPlaying ? [1, 1.6, 2.2] : [1, 1.4, 1.8],
              opacity: isPlaying ? [0.9, 0.4, 0] : [0.7, 0.3, 0],
            }}
            transition={{
              duration: isPlaying ? 1.4 : 2.2,
              repeat: Infinity,
              ease: 'easeOut',
            }}
          ...
          <motion.div
            animate={{
              scale: isPlaying ? [1, 1.5, 1.9] : [1, 1.3, 1.6],
              opacity: isPlaying ? [0.9, 0.3, 0] : [0.8, 0.2, 0],
            }}
            transition={{
              duration: isPlaying ? 1.4 : 2.2,
              delay: 0.4,
        ...
        <div className={`relative w-12 h-14 sm:w-14 sm:h-16 rounded-2xl bg-gradient-to-br from-[#2a0404] via-[#120202] to-[#0a0c10] border-2 ${isPlaying ? 'border-white shadow-[0_0_35px_rgba(220,38,38,0.95)]' : 'border-[#dc2626] shadow-[0_0_25px_rgba(220,38,38,0.6)]'} flex flex-col items-center justify-center p-1.5 transition-all duration-300 group-hover:scale-110 group-hover:shadow-[0_0_40px_rgba(220,38,38,0.85)] group-hover:border-white`}>
          ...
          {/* Playing Music Equalizer Overlay Badge */}
          {isPlaying && (
            <div className="absolute -top-1 -right-1 z-20 bg-[#b91c1c] p-1 rounded-full border border-white text-white shadow-[0_0_10px_#dc2626] animate-bounce">
          ...
          <div className="relative w-full h-8 flex items-center justify-center gap-1">
            ...
            <motion.div
              animate={{
                scaleY: isHovered || isPlaying ? 0.75 : [1, 0.9, 1],
              }}
              transition={{
                duration: isHovered || isPlaying ? 0.2 : 2.5,
                repeat: isHovered || isPlaying ? 0 : Infinity,
              }}
          ...
          <span className="text-[8px] font-mono font-black text-[#dc2626] uppercase tracking-tighter pt-0.5 group-hover:text-white transition-colors flex items-center gap-0.5">
            {isPlaying ? <Volume2 className="w-2.5 h-2.5 text-white animate-pulse" /> : 'SPIDEY'}
          </span>
        </div>

        {/* Hover Tooltip Card */}
        <div className="absolute bottom-full left-0 mb-3 opacity-0 group-hover:opacity-100 transition-all duration-300 pointer-events-none translate-y-2 group-hover:translate-y-0">
          <div className="px-3 py-1.5 rounded-xl bg-[#090b10]/95 border border-[#dc2626]/70 shadow-[0_0_20px_rgba(220,38,38,0.4)] backdrop-blur-md text-[10px] font-mono text-white whitespace-nowrap flex items-center gap-2">
            <span className={`w-2 h-2 rounded-full ${isPlaying ? 'bg-emerald-400 animate-ping' : 'bg-[#dc2626] animate-pulse'} shadow-[0_0_6px_#dc2626]`} />
            <span className="font-bold text-[#dc2626]">SPECTACULAR SPIDER-MAN</span>
            <span className="text-slate-400">| {isPlaying ? 'Click to Pause' : 'Click to Play Song'}</span>
          </div>
        </div>
      </motion.div>
    </div>
